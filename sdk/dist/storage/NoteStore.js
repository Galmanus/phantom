...
export class NoteStore {
    ...
    async decrypt(ciphertext, iv) {
        ...
        const decrypted = await crypto.subtle.decrypt({ name: 'AES-GCM', iv: ivBuffer }, this.encryptionKey, ciphertextBuffer);
    ...
    async getAllNotes() {
        ...
        for (const record of records) {
            try {
                const decrypted = await this.decrypt(record.encrypted_data, record.iv);
                notes.push(JSON.parse(decrypted));
            }
            catch (error) {
                console.error('Failed to decrypt note:', error);
            }
     ...
     */
    async saveYieldPosition(position) {
        if (!this.db)
            throw new Error('Store not initialized');
    ...
    async getAllYieldPositions() {
        ...
        for (const record of records) {
            try {
                const decrypted = await this.decrypt(record.encrypted_data, record.iv);
                positions.push(JSON.parse(decrypted));
            }
            catch (error) {
     ...
     */
    async updateIntentStatus(commitment, status) {
