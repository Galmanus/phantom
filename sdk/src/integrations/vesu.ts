...
export class VesuIntegration {
  ...
  private singleton: Contract;

  constructor(rpcUrl: string) {
    this.provider = new RpcProvider({ nodeUrl: rpcUrl });
    this.singleton = new Contract(
      VESU_ABI,
   ...
   */
  async getCurrentAPY(assetAddress: string): Promise<number> {
    try {
  ...
  async getPositionValue(params: {
    ...
    try {
      const position = await this.singleton.position_unsafe(
        params.poolId,
